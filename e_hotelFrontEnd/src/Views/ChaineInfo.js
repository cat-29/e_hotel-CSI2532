...
export const ChaineInfo = () => {
  ...
  const handleDeleteSubmit = async (e) => {
    if (state.chaineInfo.nbrHotel == 0) {
      console.log(e);
      console.log("handleDeleteSubmit sent");
      try {
        await adminService.deleteChaine(state.chaineInfo.nomChaine).then(() => {
          console.log("deleteChaine called for: ", state.chaineInfo.nomChaine);
          navigateToChaines();
        });
      } catch (error) {
        console.error(error);
  ...
  return (
    <>
      ...
      <form
        noValidate
        className="align-middle mx-5 my-2 mb-3 border p-2"
