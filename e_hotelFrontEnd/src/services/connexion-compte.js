...
class ConnexionCompteService {
  ...
  async getNumeroChambreForSpecifications(
    hotelId,
    dateCheckIn,
    dateCheckOut,
    capacite,
    ...
    return http.get(
      `/chambre/numeroChambre/${hotelId}/${dateCheckIn}/${dateCheckOut}/${capacite}/${vue}`
    );
  }
  ...
  async getChambresFromNombreDeChambres(chambreMin, chambreMax) {
    console.log(
      "value of chambre min = " + chambreMin + ", chambreMax = " + chambreMax
    );
    if (chambreMin == "" && chambreMax == "") {
      // returne toutes les chambres
      console.log(
        "il n'y a pas de chambres min et chambres max de selectionnees. Retourne tout les hotels"
      );
      return await this.getAllRooms();
    } else if (chambreMin == "") {
      // trouve juste les chambres qui satisfont la borne chambreMax
      console.log(
        "il n'y a pas de classements chambreMin de selectionnee. Retourne tout les hotels qui respectent chambreMax"
      );
      return http.get(`/chambre/getAllRooms/nbrChambreMax/${chambreMax}`);
    } else if (chambreMax == "") {
      // trouve juste les chambres qui satisfont la borne chambreMin
      console.log(
        "il n'y a pas de classements chambreMax de selectionnee. Retourne tout les hotels qui respectent chambreMin"
      );
  ...
  async getAllChambresFromClassement(classement) {
    ...
    if (classement.length == 0) {
      console.log(
        "il n'y a pas de classements selectionnees. Retourne tout les hotels"
      );
      return await this.getAllRooms();
    }
  ...
  async getAllChambresFromChaineHoteliere(arrayChaine) {
    ...
    if (arrayChaine.length == 0) {
      console.log(
        "il n'y a pas de chaines selectionnees. Retourne tout les hotels"
      );
      return await this.getAllRooms();
    }
  ...
  async getHotel(idHotel) {
    return http.get(`/hotel/${idHotel}`);
  }

  async getChaine(idHotel) {
    return http.get(`/chaine/${idHotel}`);
  }
}
